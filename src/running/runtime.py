...
class OCaml(Runtime):
    ...
    # Whether to build a per-switch olly. MMTk replaces the GC and does not
    # emit the stock runtime_events GC counters, so it disables this.
    BUILD_OLLY_IN_SWITCH = True
    ...
    def __init__(self, **kwargs):
        ...
        if executable:
            ...
            if self.BUILD_OLLY_IN_SWITCH and olly_dir_env and Path(olly_dir_env).is_dir():
                try:
...
class OCamlMMTk(OCaml):
    ...
    # MMTk doesn't emit the stock runtime_events GC counters, so a per-switch
    # olly would be useless (and building it under the sandbox would flake).
    BUILD_OLLY_IN_SWITCH = False
