"""CPU topology detection and pinning. The FreeBSD fixtures reproduce
kern.sched.topology_spec (sys/kern/sched_ule.c).
"""
import shutil
...
# 4 physical cores, SMT on, FreeBSD's interleaved enumeration
FREEBSD_SMT = """<groups>
...
# SMT off: no THREAD-flagged group
...
def _no_kernel_cpu_lists(request, monkeypatch):
    """Stub the running machine's sysfs CPU lists, so the host's own isolcpus=
    does not reach into tests with a fake topology."""
...
def test_freebsd_interleaved_enumeration_differs_from_linux(as_freebsd, monkeypatch):
    """Same policy, same hardware, different CPU list: FreeBSD interleaves siblings."""
    monkeypatch.setattr(osinfo, "probe", lambda cmd: FREEBSD_SMT)
...
def test_default_partition_matches_current_pin_lavyek_policy(monkeypatch):
    """Reproduces the hand-written `taskset -c 0-15`; changing the default changes what existing numbers mean."""
...
def test_pin_command_empty_on_macos(monkeypatch):
    # macOS has no binding CPU affinity API
    monkeypatch.setattr(osinfo, "IS_LINUX", False)
    monkeypatch.setattr(osinfo, "IS_FREEBSD", False)
...
def test_linux_topology_is_self_consistent():
    ...
    # online count, not `present`: a box with SMT disabled has half its CPUs offline
...
#
# The Ryzen fixture is real ocaml-processor-dump output; the hybrid and
# dual-socket ones are constructed.
...
# 4 P-cores with SMT (cpu0-7) + 4 single-thread E-cores (cpu8-11)
HYBRID_CPUS = (
...
def test_faked_topology_cannot_corrupt_the_kernel_view():
    """A faked flat topology (one socket, no E-cores) must filter nothing."""
...
def test_summary_reports_kinds_and_sockets(monkeypatch):
    ...
    # mixed group widths: no single answer
...
def test_summary_is_empty_where_nothing_is_knowable(monkeypatch):
    ...
    monkeypatch.setattr(osinfo, "IS_LINUX", False)
    # "none" is itself the useful fact
    assert osinfo.machine_topology_summary() == {"cpu_isolation": "none"}


# --- NUMA and socket detection -------------------------------------------------
#
# The FreeBSD fixture models a 2-socket Xeon E5-2640 v4: 20 cores, 40 threads,
# node boundary at cpu 20.
...
def test_freebsd_numa_absent_when_not_flagged(as_freebsd, monkeypatch):
    # THREAD-flagged groups must not be mistaken for NUMA nodes
    monkeypatch.setattr(osinfo, "probe", lambda cmd: FREEBSD_SMT)
...
# --- one-node pinning ----------------------------------------------------------
#
# 2 sockets x 10 cores x 2 threads, siblings adjacent, NUMA boundary at cpu 20.
...
def test_one_node_deliberately_leaves_cpus_unassigned(rosemary):
    # not a partition of every CPU: the benchmark's siblings stay idle
    bench, observers = osinfo.partition_cpus(one_node=True)
    assert not (set(bench) & set(observers))
...
def test_one_node_composes_with_reserved_cores(rosemary):
    # reserved_cores applies within the chosen node
...
def test_group_straddling_nodes_stays_with_the_benchmark(monkeypatch):
    # should not happen, but must not crash
...
def test_one_node_leaves_the_benchmarks_siblings_idle(rosemary):
    """With a spare node for the observers, the benchmark's own siblings must stay idle."""
    bench, observers = osinfo.partition_cpus(one_node=True)
...
def test_default_still_gives_observers_the_siblings(rosemary):
    # without one_node the siblings are the only spare CPUs
    bench, observers = osinfo.partition_cpus()
...
def test_every_core_isolated_is_ignored(monkeypatch):
    # an isolcpus= covering everything leaves the OS nowhere to run; ignore it and warn
...
def test_isolation_does_not_hand_observers_the_benchmarks_siblings(monkeypatch):
    # SMT on, isolcpus naming both threads of four cores
    monkeypatch.setattr(osinfo, "sibling_groups",
...
def test_offline_cpus_are_not_phantom_cores(monkeypatch, tmp_path):
    # An offline CPU has no readable thread_siblings_list and must not become a phantom core.
    base = tmp_path / "cpu"
    for c in range(4):
...
def _cpupin(monkeypatch, groups, isolated=(), **kwargs):
    ...
    # irq_cpus is stubbed empty by the autouse fixture
    monkeypatch.setattr(osinfo, "IS_LINUX", True)
    monkeypatch.setattr(osinfo, "IS_FREEBSD", False)
...
def test_unused_benchmark_cores_are_not_given_to_the_observers(monkeypatch):
    # unused benchmark cores stay idle, not given to olly
    m = _cpupin(monkeypatch, [[c] for c in (0, 2, 4, 6, 8, 10, 12, 14)],
                isolated=(4, 6, 8, 10, 12, 14), bench_cores="1")
...
def test_bench_cores_avoids_cpu0_where_nothing_is_declared(monkeypatch):
    # taken from the far end, away from CPU 0
...
def test_isolation_wins_over_irq_affinity(monkeypatch):
    # isolation is the stronger statement
