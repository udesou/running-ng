...
experiment in YAML (which compilers, which benchmarks, which GC parameters) and
it builds the compilers, builds the benchmark binaries against each one, runs
them under `perf`/`pmcstat` and `olly`, and writes structured results.
...
| Repo | What | Size |
|---|---|---|
| [**benches**](https://github.com/ocaml-bench/benches) | sandmark-derived microbenchmarks (sequential, multicore, effects, numerical) | 13 suites, 196 programs enabled |
| [**macro-benches**](https://github.com/ocaml-bench/macro-benches) | real-world OCaml applications in one vendored dune monorepo (menhir, coq, alt-ergo, frama-c, cpdf, jsoo, ...) | 21 active suites, 95 programs enabled |

Both work standalone. running-ng adds per-runtime opam switch management,
modifier composition (GC knobs, perf counter groups, CPU pinning), parameter
sweeps, and machine-readable output.
...
`install_deps_macos.sh` (Homebrew) or `install_deps_freebsd.sh` (pkg, and it
assumes no root). It installs system packages, opam 2.2+, a tools switch, `olly`
(built from `~/runtime_events_tools`), `pyyaml`, and clones `benches/` next to
this repo if it is not there.

The minimum is Python 3 with `pyyaml`, opam 2.2+, and
...
olly and rusage still produce data. See [Hardware counters](#hardware-counters).
...
Two cheap benchmarks, one runtime, one invocation. The first run builds an OCaml
5.4.1 opam switch from source, so budget time for that; afterwards it is a minute
or two.
...
From there, swap `CONFIG_FILE` for a real experiment, for example
`src/running/config/experiments/macrobenchmarks_monorepo.yml` (cross-runtime at
default GC) or `experiments/fp_flambda_macrobenchmarks.yml` (frame-pointer by
flambda, 2x2).

> `run_ocaml_bench_gc_sweep.sh` treats `RUNNING_BENCH_DIR` and
> `RUNNING_MACRO_BENCH_DIR` as synonyms and falls back to `../benches` lazily, so
> setting either one is enough for a macro-only run.
...
`<hfac>`/`<size>` are `0` for OCaml runs (heap-fraction fields inherited from the
JVM lineage). With `compress_logs: true`, the default when a config does not say
...
tool's output verbatim. No custom parsers are involved. The `perf` key is used
...
**Layering is the model.** An experiment file `includes:` a base and declares only
...
Merge rules (the failure modes are in [CLAUDE.md](CLAUDE.md)):

- Lists from base and experiment are **concatenated**; dicts are **merged**.
- A **top-level scalar** the base already sets (`invocations`, `schema_version`,
  `compress_logs`, `remote_host`, `minheap_multiplier`, ...) must be changed
  through `overrides:`, not redeclared at top level.
- A top-level `benchmarks:` block **extends** the base's; to *replace* it, put it
  under `overrides:`.
...
experiment) switches on **native data-contract emission**: `contract/` is written
as the run proceeds. Without it, the run is legacy-only and needs a post-hoc
`running adapt`.
...
whatever a previous run installed. That costs 10 to 20 minutes per runtime, so for
long sweeps over switches you trust set `RUNNING_REUSE_SWITCHES=1`; reuse mode
still refuses a switch whose compiler never finished building. Two runs sharing an
opam root are refused outright (`$OPAMROOT/running-ng.lock`), because one would
delete a switch the other is using: give concurrent campaigns separate
`OPAMROOT`s. Reuse-mode runs take a shared lock and may overlap. Whatever switch
was active before the run is restored afterwards, even on a crash. The reasoning
behind each of these is in [CLAUDE.md](CLAUDE.md).
...
(`running-ng-oxcaml-build`) so it cannot disturb yours.
...
sandbox so cargo can fetch crates). [CLAUDE.md](CLAUDE.md) says why each is
needed.
...
completes in; MMTk is the only runtime that supports it. Read those numbers with
[CLAUDE.md](CLAUDE.md)'s caveats in hand: off-heap memory does not count against
the budget, and a few benchmarks crash under MMTk and are excluded from the
shipped configs.
...
global shadows them. The parallel (lavyek) suite is the exception and still needs
`|re_par-22|md_par-8|pin_lavyek` on the config string.
...
status, are in [CLAUDE.md](CLAUDE.md) and `docs/freebsd-support.md`.
...
Useful `runbms` flags: `-i N` (override invocations), `--resume <run-id>` (skip
cells whose log already exists), `--skip-oom N` / `--skip-timeout N`, `-p PREFIX`
(prefix the run id), `-d` (dry run), `-v` (debug logging).
...
- a real-world application with a dependency tree → **`~/macro-benches`**, which
...
The build must be **hermetic per runtime**: unset inherited opam/OCaml variables
that could leak another switch's `.cmi` files, and build into a per-runtime
directory (`--build-dir _build-${RUNNING_OCAML_RUNTIME_NAME}`) so runtimes do not
clobber each other. macro-benches does both; see
[benches](https://github.com/ocaml-bench/benches#build-script-contract) and
[macro-benches](https://github.com/ocaml-bench/macro-benches#build-script-contract)
for the full templates.

Make it **long enough to measure**: aim for roughly 5 to 30 s. If a single
invocation is shorter, add an in-process iteration count read from `argv` or an
environment variable rather than looping the binary in a shell, because `olly`
attaches to one process and a shell loop silently measures only the first child.
...
**`expected_exit`** defaults to 0 and you need it only when a successful run exits
non-zero (today only `alt_ergo_unsat_smt2`, which runs with `--timelimit 15` and
dies of its own SIGVTALRM with 142). Without it the runner treats the run as a
crash and the invocation is **dropped from the contract**, so the benchmark
silently never reaches the dashboard. Keep it in sync with macro-benches'
`benchmarks/manifest.yml`.

### 3. Tag it (macro only)

If it exercises a runtime mechanism on its hot path, add it under that tag's
`exercised_by:` in the `tags:` block, with a `verified_at:` citation. That is what
`RUNNING_TAG` selects on, and `validate_tags()` rejects a stale reference on the
next run. Uses that exist but are not hot go under `cold:`. Add it to the ladder
tags (`default_run` and friends) too.
...
Check the `olly_*.json` sidecar for a plausible `wall_time` and no lost events. If
...
An experiment is one YAML file under `src/running/config/experiments/`. It
`includes:` a base and declares only `runtimes`, `configs`, `modifiers`,
`config_sweep`, `comparisons` and `overrides`.
[`examples/ocaml_gc_sweep_example.yml`](src/running/config/examples/ocaml_gc_sweep_example.yml)
is the annotated template; of the real ones,
`offheap_M_o_sweep_5.4.1_vs_d8bb46c.yml` is the nearest thing to a sweep and
`macrobenchmarks_monorepo.yml` to a plain cross-runtime comparison. Head the file
with the question it answers, the grid, the expected cost and the exact command
that reproduces it.
...
- **Every runtime must appear in `configs:`**, and, if you declare `comparisons:`,
  in a comparison too. Dead declarations and uncovered runtimes are validation
  errors, not warnings.
- **If your `benchmarks:` block enables the parallel (lavyek) suite**, every config
  string must carry `|re_par-22|md_par-8|pin_lavyek`; the `_par` modifiers require
  explicit values.
- **Reduce the grid before you commit to it.** `-d` dry-runs the whole expansion,
  and `-i 1` with a two-benchmark `overrides.benchmarks` gives a cheap end-to-end
  check. Note that `-d` still provisions compilers.

Then run it, and record what came out; the experiment file's header comment is the
natural home for the command line and any caveats.
...
Long sweeps are worth resuming rather than restarting: `--resume <run-id>` skips
cells whose log file already exists. Run output (`gc-sweep-logs*/`) is gitignored.
...
run. That is the input to
[ocaml-bench-dashboard](https://github.com/udesou/ocaml-bench-dashboard), which
validates it and renders regression, GC, absolute-value and sweep views. Older
runs are converted with `running adapt <run-dir>`, which shells out to
`contract-adapter/` (build it once with `contract-adapter/build.sh`).

**Notebooks.** `notebooks/` holds three Jupyter analyses over the raw sidecars (A:
regression dashboard, B: runtime-behaviour explorer, C: GC parameter sweep),
...
**Quick plots.** `python3 scripts/plot_gc_sweep.py <run-dir>` writes a CSV summary
and sweep plots straight from a log directory.
...
Contributor conventions, the internals that matter when something breaks, the
platform-support detail and the current list of known-broken configs are in
[CLAUDE.md](CLAUDE.md). `docs/` contains upstream's mdBook (JVM-oriented), this
fork's benchmark methodology notes, and `freebsd-support.md`.
